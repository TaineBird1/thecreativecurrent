...
**Shared components** (used by both admin and portal — moved to `src/components/` mid-build once that became clear): `LiveVisitorCount.tsx`, `TrafficChart.tsx` (dependency-free CSS bar chart), `RevenueTracker.tsx` (see below), `ChangeRequestList.tsx` (takes optional `customerId`/`canUpdateStatus`/`showBusinessName` props to serve both the customer's own list and the admin's cross-customer checklist).

**Revenue tracking**: a third `analytics_events.event_type`, `conversion`, plus `value` (numeric) and `label` (text) columns — what a customer's *own* site earns them (a sale, a booking), not what they pay the agency (this project doesn't track that at all). Nothing infers a sale from browsing alone: the client's site (or its e-commerce/booking checkout success page) has to explicitly call `window.tccTrackConversion(value, label)`, a function `track.js` now exposes alongside its automatic pageview/heartbeat calls. `RevenueTracker.tsx` shows a 30-day total (via the `customer_revenue(customer_id, days)` SQL function, `SECURITY INVOKER` like `live_visitor_count` so RLS still gates it) plus the 5 most recent conversions, and renders identically on both `PortalDashboard.tsx` (the customer's own view) and `AdminCustomerDetail.tsx` (the admin's per-customer view). No real e-commerce-platform integration (Shopify, WooCommerce, etc.) exists — that would be a separate, platform-specific webhook/OAuth build; this is just the tracking primitive a client's developer (or the agency) can wire a checkout page into.
...
- `src/lib/outreachTemplate.ts` — `buildOutreachDraft(businessName, category, reason, defect?, openingOverride?)`, a pure string template: fixed pitch + signature, with the opening paragraph either rule-based (a different canned line for `poor_website` vs `no_website`, or the specific `email_defect` when one exists) or, when `openingOverride` is supplied, an AI-written one — see AI-Personalized Outreach below. No longer entirely client-side: the AI path needs the Anthropic API key, so drafting now goes through `api/prospects-draft.ts` rather than running in the browser.
...
`api/_lib/aiDraft.ts`'s `generateAiOpening()` calls Claude (`claude-opus-5`, `output_config.effort: "low"`, structured JSON output) to write just the opening paragraph of a draft — grounded in the same facts the rule-based opener already had (`websiteHealth.ts`'s `emailDefect`, category, no-website vs poor-website) so it's a rewrite of the hook, not a new source of claims about the prospect. The pitch and signature stay `outreachTemplate.ts`'s fixed copy, assembled around whichever opening wins. Returns `null` on any failure (missing `ANTHROPIC_API_KEY`, rate limit, malformed response), and every caller — `api/outreach-run.ts` and `api/prospects-draft.ts` (the endpoint `ProspectCard.tsx`'s "Generate draft" button now calls, since an API key can't live in the browser bundle) — falls back to the rule-based opener in that case, so an LLM outage can never be the reason a lead fails to draft.

### Reply Handling (`/admin/outreach/replies`)

`api/check-replies.ts` polls the admin's Gmail inbox via IMAP (`imapflow`, `imap.gmail.com:993`, same `GMAIL_USER`/`GMAIL_APP_PASSWORD` app password used for sending — Gmail app passwords work for both SMTP and IMAP) and matches replies back to `status = 'sent'` prospects by sender address, one search per candidate rather than a single broad inbox scan. On a match: parses the message with `mailparser`, sets `status = 'replied'` with `reply_body`/`reply_received_at`/`reply_message_id`, and calls `api/_lib/aiReply.ts`'s `generateSuggestedReply()` (same Claude setup as the draft opener) to fill `ai_suggested_reply`, grounded in the exact original draft and the exact reply text. Two entry points, same shape as `outreach-run.ts`: `GET` on a second daily cron (`0 14 * * *`, a few hours after the 5am outreach run) checked against `CRON_SECRET`, and `POST` via the "Check now" button on `/admin/outreach/replies` checked via `requireAdmin`. Nothing sends automatically — `src/admin/AdminOutreachReplies.tsx` shows the reply and the AI suggestion (editable) per prospect, and `api/prospects-send-reply.ts` sends whatever's currently in that edit box on explicit click, via a new `sendOutreachReply()` in `api/_lib/email.ts` that carries `In-Reply-To`/`References` (so it threads in the recipient's inbox) and skips the POPIA opt-out footer (a reply to a conversation the prospect started isn't unsolicited marketing).
